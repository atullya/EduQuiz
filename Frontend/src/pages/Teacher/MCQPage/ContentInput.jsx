  ...
  if (formData.platform === "text") {
    return (
      <div className="space-y-3 animate-in slide-in-from-top-5 duration-500">
        ...
        />
        <p className="text-sm text-gray-500">💡 Tip: Provide detailed content for better question generation</p>
      </div>
    )
  }
        ...
        <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-purple-400 transition-colors">
          ...
          {formData.pdfFile && (
            <div className="mt-4 p-3 bg-green-50 rounded-lg">
              <p className="text-green-800 font-medium">✓ {formData.pdfFile.name}</p>
            </div>
          )}
        </div>
      </div>
